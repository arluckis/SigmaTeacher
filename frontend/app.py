...
if st.session_state.mostrar_professor:
    ...
    with aba_historico:
        ...
        if audios:
            ...
            for idx, audio in enumerate(audios):
                with st.expander(
                    f"📝 {audio['filename_original']} - ID: {audio['id']}", expanded=False
                ):
    ...
    # ====================================================================
    # ABA 4: SigmaTeacher CHAT (Sessão de Tutoria)
    # ====================================================================
    with aba_chat:
        ...
        if not st.session_state.chat_iniciado or not st.session_state.session_id:
            st.info(
                "📌 Nenhuma sessão ativa. Configure e inicie uma sessão na aba 'Configuração do ITS'."
            )
        else:
